...
class SurveyGeometryPlot(PairedPlot):  # pylint: disable=too-many-instance-attributes
    ...
    * Receiver view: displays locations of receivers. Highlights all sources that activated the receiver on click and
    ...
    def __init__(self, survey, show_contour=True, keep_aspect=True, source_id_cols=None, source_sort_by=None,
        ...
        survey = survey.clone()
        if source_id_cols is not None:
            survey.set_source_id_cols(source_id_cols)
        if receiver_id_cols is not None:
            survey.set_receiver_id_cols(receiver_id_cols)

        source_headers = survey.source_headers
        self.source_map = MetricMap(coords=source_headers[["SourceX", "SourceY"]],
                                    values=np.ones(survey.n_sources),
                                    index=source_headers[to_list(survey.source_id_cols)])
        self.source_survey = survey.reindex(survey.source_id_cols)
        ...
        receiver_headers = survey.receiver_headers
        self.receiver_map = MetricMap(coords=receiver_headers[["GroupX", "GroupY"]],
                                      values=np.ones(survey.n_receivers),
                                      index=receiver_headers[to_list(survey.receiver_id_cols)])
        self.receiver_survey = survey.reindex(survey.receiver_id_cols)
